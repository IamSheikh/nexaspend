...
const AddTransactionAccountModal = ({
  setIsModalOpen,
  setRefreshState,
}: {
  setIsModalOpen: any;
  ...
  const [accountInputData, setAccountInputData] = useState<ITransactionAccount>(
    {
      accountName: '',
      balance: 0,
      // @ts-ignore
  ...
  const handleModalSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (accountInputData.accountName === 'Cash') {
      setError(
        `Account name Cash is not allowed. Please choose a different name for your account.`,
      );
    } else {
      await window.electron.addTransactionAccount(accountInputData);
        ...
        <form onSubmit={handleModalSubmit}>
          ...
          <div className="flex items-center space-x-2 mt-4 w-full">
            ...
            <input
              ...
              required
              value={accountInputData.accountName}
              onChange={(e) => {
